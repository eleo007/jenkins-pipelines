library changelog: false, identifier: 'lib@PSQLADM-333', retriever: modernSCM([
    $class: 'GitSCMSource',
...
void runPlaybook(String action_to_test) {
    ...
    sh '''
        git clone -b PSQLADM-333 --depth 1 "${git_repo}" --single-branch
    '''
...
pipeline {
    ...
    stages {
        stage("Prepare") {
            steps {
                script {
                    currentBuild.displayName = "#${BUILD_NUMBER}-${params.product_to_test}-${params.install_repo}-${params.node_to_test}-${params.client_to_test}"
                }
            }
        }
